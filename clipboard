...
# https://stackoverflow.com/questions/4597937/perl-function-to-trim-string-leading-and-trailing-whitespace
# TODO: possibly turn multi-line into single spaces?
sub trim {
    (my $s = $_[0]) =~ s/^\s+|\s+$//g;
    return $s;
}
...
sub paste {
    ...
    # trim strings so commands do not get run accidently
	$str = trim($str);
	if ($? == 0) {
		$self->tt_paste($str);
	} else {
